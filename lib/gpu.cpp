								   ...
								   const int x2, const int y2, const Colour &colour)
{
	if (where >= GPU_RENDER_LINE1 && where <= GPU_RENDER_LINE2)
		DrawLine(x1, y1, x2 + 1, y2 + 1, {colour.r * 0x11, colour.g * 0x11, colour.b * 0x11, 0xFF});
	else if (where >= GPU_RENDER_RECT1 && where <= GPU_RENDER_RECT2)
		DrawRectangle(x1, y1, x2 - x1 + 1, y2 - y1 + 1, {colour.r * 0x11, colour.g * 0x11, colour.b * 0x11, 0xFF});
}
